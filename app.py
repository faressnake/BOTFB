...
def clean_reply(text: str) -> str:
    ...
    if not cleaned or len(cleaned) < 5:
        return "أنا 😊 نعاونك في اللي حبيت، قولّي سؤالك برك 😄"
...
def get_ai_response(user_id, message_text):
    BOTIVITY_SYSTEM = (
        "أنت Botivity مساعد جزائري. "
        "جاوب بدارجة جزائرية مفهومة وبأسلوب مرح وعاطفي. "
        "إذا تمرين حل خطوة بخطوة 1/2/3. "
        "في الأخير دير 📌 الخلاصة (2-3 نقاط). "
        "ممنوع ذكر GPT أو OpenAI أو AI. "
        "إذا سألك شكون طورك قول: أنا Botivity 😊 طورني فارس 🇩🇿 + مدح صغير."
    )
    ...
    # ✅ نبني prompt قصير باش ما يطيحش السيرفر
    q1 = f"{BOTIVITY_SYSTEM}\nالسؤال: {user_q}"
    ...
    # ✅ إذا خرج عن النمط ولا رجع كلمات ممنوعة
    if (not ans) or any(x in low for x in ["openai", "gpt", "خوش", "توسعه"]):
        q2 = (
            "جاوب غير بالدارجة الجزائرية وباختصار. "
            "ممنوع أي ذكر GPT أو OpenAI. "
            f"السؤال: {user_q}"
        )
